...
- Column references may be unqualified (`amount`) or qualified (`orders.amount`). Unquoted identifiers are case-insensitive; double-quoted ones are case-sensitive. Upstream's validator tries `parse_one(expr)` then falls back to `parse_one(f"SELECT {expr}")`; on sqlglot ≥30 a bare column name already parses as a `Column`, so the fallback rarely fires — keep it for parity, don't write tests that rely on it firing.
